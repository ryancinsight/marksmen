    ...
    for r in &refs {
        ...
        if let Some(bib) = &style.bibliography {
            if let Some(layout) = &bib.layout {
                outputs.push(format!("<div class=\"csl-entry\">{}</div>", evaluate_layout(layout, &ctx)));
                continue;
            }
        }
        outputs.push(format!("<div class=\"csl-entry\">{}</div>", r.title.clone().unwrap_or_default()));
    }
...
#[wasm_bindgen]
pub fn execute_mail_merge(template_markdown: &str, csv_data: &str, format: &str) -> Result<js_sys::Uint8Array, JsValue> {
    ...
    let headers = reader.headers().map_err(|e| JsValue::from_str(&e.to_string()))?.clone();
