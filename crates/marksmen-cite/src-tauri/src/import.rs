...
pub async fn import_pdf(app: tauri::AppHandle) -> Result<Option<Reference>, String> {
    ...
    if let Ok(meta) = extract_pdf_metadata(&bytes) {
        if let Some(t) = meta.title.filter(|t| !t.is_empty()) {
            r.title = t;
        }
                ...
                _ => {
                    if let Some(r) = cur.as_mut() {
                        match tag {
                            ...
                            "SP" => {
                                if r.pages.is_empty() {
                                    r.pages = val.to_string();
                                } else {
                                    r.pages = format!("{}-{}", val, r.pages);
                                }
                            }
                            "EP" => {
                                if r.pages.is_empty() {
                                    ...
                                    r.pages = format!("{}-{}", r.pages, val);
                                }
                            }
                            "PB" => r.publisher = val.to_string(),
                            "SN" => {
                                if val.len() == 13 || val.len() == 10 {
                                    r.isbn = val.to_string();
                                } else {
                                    r.issn = val.to_string();
                                }
                            }
        ...
        for line in entry.lines() {
            ...
            if let Some(eq) = line.find('=') {
                ...
                match key.as_str() {
                    ...
                    "author" => {
                        r.authors = val.split(" and ").map(|s| s.trim().to_string()).collect()
                    }
