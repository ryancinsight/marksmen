...
fn format_csl_bibliography(
    ...
    for citation in citations {
        ...
        let author_str = ref_model
            .author
            .as_ref()
            .map(|a| {
                a.iter()
                    .filter_map(|n| n.family.clone())
                    .collect::<Vec<_>>()
                    .join(", ")
            })
            .unwrap_or_else(|| "Unknown".to_string());

        let year_str = ref_model
            .issued
            .as_ref()
        ...
        let item_html = format!(
            "{} ({}). {}. <em>{}</em>.",
            author_str, year_str, title_str, pub_str
        );
...
async fn export_binder(
    ...
    let (bytes, ext): (Vec<u8>, &str) = match format.as_str() {
        "docx" => (
            marksmen_docx::translation::document::convert(
                unified_events,
                &config,
                std::path::Path::new("."),
                None,
        ...
        "pdf" => {
            ...
            let typst = marksmen_typst::translator::translate(unified_events, &config)
                .map_err(|e| e.to_string())?;
            // Then compile PDF using typst_library
            (
                marksmen_pdf::rendering::compiler::compile_to_pdf(
                    &typst,
                    &config,
...
fn load_marksmen_cite_db(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let db_path = local_data
        .join("com.ryancinsight.marksmen-cite")
        .join("cite_library")
