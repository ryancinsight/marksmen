...
//!
//! `parse_pdf` operates on two mutually exclusive paths:
//!
//! 1. **Embedded-markdown path** (lossless): When a PDF was generated by `marksmen-pdf`,
//!    the original Markdown source is embedded verbatim in the `Info` dictionary under the
//!    key `MarksmenRoundtripMarkdown`.  `parse_pdf` detects this key and returns the
//!    embedded string directly, with foreign annotations appended.  This path preserves
//!    all Markdown formatting marks (bold, italic, code, math, tables, headings) at 100%
//!    fidelity — the roundtrip similarity is bounded below only by annotation injection.
//!
//! 2. **Native-reader path** (heuristic, foreign PDFs): When the embedded key is absent,
//!    `pdf_to_markdown` is called.  It uses lopdf to walk every page content stream,
//!    decodes glyphs via the full font encoding pipeline (WinAnsi/MacRoman/CMap), applies
//!    glyph advance widths and graphics state, then clusters RichSpans geometrically into
//!    lines, paragraphs, and Markdown structure.  This path is inherently lossy for rich
//!    formatting (bold/italic require font-name inference; inline math cannot be recovered).
//!
//! **Invariant**: the embedded-markdown path is always preferred.  The native-reader path
//! is only reached when the PDF was not produced by `marksmen-pdf`.
//!
//! ## Native reader limitations
//!
//! - **Type3 fonts**: glyph procedures are not executed; Type3-rendered glyphs are skipped
//!   with a `tracing::warn!`.
//! - **XObject recursion depth**: capped at 16 levels to prevent stack overflow on
//!   circular/deeply nested Form XObjects.
//! - **Annotation position**: injected using a byte-offset-to-page-fraction heuristic.
//!   The rendered page length does not correlate linearly with source byte offset for
//!   multi-page documents with variable-density pages.
//! - **Inline math**: cannot be recovered from rendered glyphs; only textual approximation.
//!
...
pub fn extract_annotations(bytes: &[u8]) -> Result<Vec<LocalizedAnnotation>> {
    ...
    for (page_num, &page_id) in pages.iter() {
        ...
        // We will collect foreign annotations and their raw dicts to process them later.
        // This allows us to skip the expensive text extraction if all annotations are native.
        let mut foreign_annots = Vec::new();
        ...
        for annot_obj in annot_list {
            ...
            // Skip our own origin annotations immediately.
            ...
            foreign_annots.push(annot_dict);
        }

        if foreign_annots.is_empty() {
            continue;
        }

        // Lazily build text mapper for this page only when needed for anchoring foreign annotations.
...
/// Parses raw PDF bytes and reconstructs a Markdown string.
///
/// Selects the lossless embedded-markdown path when the `MarksmenRoundtripMarkdown` key is
/// present in the PDF `Info` dictionary (marksmen-generated PDFs), otherwise falls back to
/// the heuristic native-reader path (foreign PDFs).  See module-level docs for full semantics.
///
/// Foreign PDF annotations are extracted and appended as text-anchored `<mark>` tags.
