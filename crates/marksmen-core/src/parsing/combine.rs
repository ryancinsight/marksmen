        ...
        for event in doc_events {
            let namespaced_event = match event {
                ...
                Event::Start(Tag::Heading { level, id, classes, attrs }) => {
                    let new_id = id.map(|i| CowStr::Boxed(format!("{}-{}", namespace, i).into_boxed_str()));
                    Event::Start(Tag::Heading { level, id: new_id, classes, attrs })
                }
                
                // Namespace Link references (e.g. [Link](#my-heading) -> [Link](#namespace-my-heading))
                Event::Start(Tag::Link { link_type, dest_url, title, id }) => {
                    let new_url = if dest_url.starts_with('#') {
                        CowStr::Boxed(format!("#{}-{}", namespace, &dest_url[1..]).into_boxed_str())
                    } else {
                        dest_url
                    };
...
mod tests {
    ...
    fn test_ast_concatenator() {
        ...
        // Assert page break
        assert!(combined.iter().any(|e| matches!(e, Event::Html(h) if h.contains("page-break"))));
        ...
        assert!(combined.iter().any(|e| matches!(e, Event::Html(h) if h.contains("id=\"doc1-fig1\""))));
        assert!(combined.iter().any(|e| matches!(e, Event::Html(h) if h.contains("id=\"doc2-fig1\""))));
    }
}
